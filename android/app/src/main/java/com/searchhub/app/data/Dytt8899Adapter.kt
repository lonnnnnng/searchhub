...
/**
 * BT电影天堂 dytt8899.com (帝国CMS)
 * 搜索: POST /e/search/index.php  classid=0&show=title,smalltext&tempid=1&keyboard={kw}
 * 结果: a[href*=".html"] 带 title 属性(标题含年份)
 * 详情: /i/{id}.html 或 /i/yyyyMMdd/{id}.html
 */
class Dytt8899Adapter(override val config: SiteConfig, private val engine: HttpEngine) : SiteAdapter {
    override val id = "dytt8899"
    override val displayName = "电影天堂"
    ...
    override suspend fun search(kw: String, page: Int): List<SearchResult> = withContext(Dispatchers.IO) {
        ...
        // 结果: a[href*=".html"] + title 属性
        for (a in doc.select("a[href*=.html][title]")) {
            val href = a.attr("href")
            val title = a.attr("title").trim()
            if (title.isBlank()) continue
            // 过滤栏目/无关链接
            if (!title.contains("《") && !Regex("\\d{4}").containsMatchIn(title)) continue
            val year = Regex("(19\\d{2}|20\\d{2})").find(title)?.groupValues?.get(1) ?: ""
            val quality = Regex("(4K|1080P|1080p|720P|720p|蓝光|HD|BD|DVD)", RegexOption.IGNORE_CASE).find(title)?.groupValues?.get(1) ?: ""
    ...
    override suspend fun detail(detailUrl: String): DetailInfo = withContext(Dispatchers.IO) {
        ...
        val doc = Jsoup.parse(html)
        val title = doc.select("h1, #title, .title_all h1").firstOrNull()?.text() ?: doc.title()
