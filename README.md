...
- 🔄 **应用内自更新**：设置页在线检测新版本，下载 APK 后直接拉起系统安装器升级
...
| 4K指南 4kzn.cc | GET /?post_type=book&s= | 夸克 / 迅雷 / 光鸭云盘直链 |
| 悠悠MP4 uump4.cc | GET /search.htm?keyword= | 种子直链(.torrent) |
| 磁力熊 cilixiong.xyz | 帝国CMS POST (classid=1,2) | 1080P 磁力直链 |

> 已下线：短剧狗（btwaf JS 挑战拦截非浏览器请求，搜索恒为空）。无法接入：mini4k.com / imax4k.com（Cloudflare JS 挑战，非浏览器请求被拦，待复测）。部分条目比特大雄新片源站磁力为占位符、云集部分资源需登录源站，属于站点侧数据/权限限制。
...
| v1.8.0 | 紧凑化打磨：结果卡片一行元信息(站名/年份/清晰度合并)、详情页资源类型汇总+网盘提取码一键复制+源网页地址溯源、设置页单行站点布局、修复电影港导航链接混入搜索结果、移除代理功能 |
| v1.8.1 | 站点治理：比特大雄老条目二次解析修复、下线短剧狗、BT影视同名转贴去重 |
| v1.8.2 | 应用更名"聚合搜" + 绿底搜索镜新图标 |
| v1.8.3 | 修复新版6v搜索结果混入热门推荐(与关键词无关) |
| v1.8.5 | 设置页新增在线检测更新，应用内下载 APK 并拉起安装器完成升级 |
| v1.8.6 | 设置页重构为一级菜单(站点来源/检测更新/清除缓存)，站点列表移入二级页 |
| v1.8.7 | 新增对接磁力熊(豆瓣高分 1080P 磁力直链) |
| v1.8.8 | 站点来源页动作按钮(恢复默认/保存)移至标题右侧, 移除底部按钮栏 |
